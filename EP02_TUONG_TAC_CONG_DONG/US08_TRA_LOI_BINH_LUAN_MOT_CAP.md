...
4. Reply tối thiểu **1 ký tự hợp lệ hoặc 1 emoji**, tối đa **1000 ký tự**; emoji-only hợp lệ. Trước khi đếm ký tự, moderation và lưu record, UI/API áp dụng **đúng blocklist Unicode của US04 AC2**: control `U+0000–U+001F`, `U+200B`, `U+200C`, `U+200E`/`U+200F`, `U+202A–U+202E`, `U+2066–U+2069`, `U+FEFF`. `U+200D` (ZWJ) và `U+FE0F` chỉ hợp lệ khi nằm trong emoji sequence hợp lệ theo Unicode RGI.
5. URL trong reply chỉ hợp lệ với `mytv.com.vn` hoặc subdomain hợp lệ; áp dụng cùng hostname validation US04.
6. Comment + reply dùng chung rate limit **5 attempt trong rolling 60 giây/user** theo US04 AC4: attempt tính khi request đã qua validation cú pháp và được gửi tới AI moderation. Validation fail và Unicode reject không tính attempt; AI mức Nặng chặn thì không tạo reply record nhưng vẫn tính attempt và ghi `blocked_attempt`; AI lỗi/timeout **vẫn tạo record ở trạng thái Chờ duyệt Admin** theo US11 AC6.
...
12. Root không còn public thì thread không được hiển thị độc lập khỏi root. **Admin Ẩn root** làm toàn thread tạm không public nhưng không đổi moderation state của reply. Đây là **visibility cascade**: reply **không lưu cờ gate nào**; Effective Visibility Resolver tính gate active tại thời điểm đọc từ state của root, lock của tác giả/root author và status của scope theo mô hình dữ liệu tại **US12**. Nhiều gate có thể cùng active; gỡ một gate không làm mất gate còn lại. Undo Ẩn root làm các reply hợp lệ hiện lại theo state riêng nếu không còn gate nào khác, và không phụ thuộc retention delete của root.
...
- **Visibility cascade** do Admin Hide hoặc Account Lock của root author không tự thay đổi state moderation của reply và **không ghi trường nào lên hàng reply**: gate active là giá trị Resolver tính runtime theo mô hình dữ liệu tại US12 — cố ý không denormalize xuống reply, vì gate scope là thuộc tính của phim/tập và nhân bản nó xuống từng reply sẽ tạo dữ liệu lệch mỗi lần Admin Đóng/Mở scope. Visibility cascade **không có retention delete riêng và không dùng `cascade_source`**. Reply self-delete riêng lẻ có retention 90 ngày tính từ lúc xóa.
