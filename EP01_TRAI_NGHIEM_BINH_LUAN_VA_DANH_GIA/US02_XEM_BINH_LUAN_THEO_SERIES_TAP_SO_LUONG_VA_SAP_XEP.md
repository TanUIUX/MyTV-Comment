...
7. FeaturedScore của root không ghim: `0.5×ln(1+Like) + 0.3×ln(1+Reply) + 0.2×e^(-AgeHours/72)`. Sort `Nổi bật`: FeaturedScore giảm dần → `created_at` giảm dần → `comment_id` giảm dần.
8. `Mới nhất`: `created_at` giảm dần → `comment_id` giảm dần. `Nhiều lượt thích`: public Net Like giảm dần → `created_at` giảm dần → `comment_id` giảm dần.
...
12. Có comment mới khi đang đọc: không tự chèn; hiện `Có {n} bình luận mới`. Khi refresh, `Nổi bật`/`Nhiều lượt thích` luôn giữ đúng ranking; `Mới nhất` phải đưa focus tới comment mới nhất đầu tiên.
...
- Nội dung tạm non-public do **Account Lock** của tác giả/root author (kể cả reply cascade theo root) và do **scope Đóng bình luận** đều **không** được tính vào tổng số công khai hiển thị cho end-user tại thời điểm đọc. Đây là **visibility gate** theo thứ tự ưu tiên của Effective Visibility Resolver (US12), không đổi `moderation_state` lưu trữ của comment. Về KPI: **Account Lock tạm loại** content/thread khỏi public KPI và Engagement Score; **scope Đóng không loại** KPI/Engagement Score vì chỉ là gate hiển thị vận hành (US19).
...
| TC-US02-011 | Featured tie | Hai root có Featured Score bằng nhau, có trường hợp cùng `created_at` | Mở sort `Nổi bật` nhiều lần/lazy load | FeaturedScore giảm dần; tie dùng `created_at` giảm dần; nếu vẫn bằng nhau dùng `comment_id` giảm dần, thứ tự ổn định giữa các lần load. |
| TC-US02-012 | Latest | Có root đăng ở nhiều thời điểm khác nhau và có trường hợp cùng `created_at`, kèm reply | Chọn sort `Mới nhất` | Root sắp theo `created_at` giảm dần rồi `comment_id` giảm dần; reply không bị trộn thành item gốc trong list. |
| TC-US02-013 | Most liked | Có root với Net Like khác nhau và trường hợp Net Like + `created_at` bằng nhau | Chọn sort `Nhiều lượt thích` | Public Net Like giảm dần; tie dùng `created_at` giảm dần rồi `comment_id` giảm dần. |
...
| TC-US02-017 | New comments | Đang đọc ở sort `Nổi bật`, `Nhiều lượt thích` và `Mới nhất`; có comment mới được đăng | Quan sát list rồi bấm indicator | Không tự chèn comment mới vào list; hiện `Có {n} bình luận mới`. Khi refresh, `Nổi bật`/`Nhiều lượt thích` giữ đúng ranking; `Mới nhất` bắt buộc đưa focus tới comment mới nhất đầu tiên. |
...
|---|---|
| Lựa chọn sort `Nổi bật` | **Nổi bật**<br>Ưu tiên bình luận có tương tác cao và còn mới.<br>`[Chọn]` |
| Ghi chú Pin | **Bình luận được ghim luôn ở đầu mọi cách sắp xếp.** |
