...
5. Mọi thay đổi threshold/policy có audit và chỉ áp dụng từ thời điểm hiệu lực. Khi request Comment/Reply/Edit được nhận, hệ thống snapshot `mode + scope + threshold/policy version`; AI response dùng snapshot này dù trả sau effective time, record lưu snapshot để audit và không tự reclassify khi policy đổi.
...
| TC-US11-013 | Edit rate limit | U1 có C1/R1 | Gửi 5 version sửa trên cùng target trong rolling 60 giây rồi gửi lần thứ 6 | 5 lần đầu đi qua pipeline moderation bình thường; lần thứ 6 trên cùng target bị chặn trước khi gọi AI/tạo queue item; sau khi edit cũ nhất ra khỏi rolling window thì được thử lại. C1 và R1 có quota độc lập. |
...
| TC-US11-017 | Policy snapshot | Request Comment/Reply/Edit nhận trước effective time T; policy cũ snapshot; AI trả sau T | Đổi threshold/mode tại T rồi nhận AI response | Decision dùng mode/scope/threshold/policy version snapshot lúc nhận request; record/audit lưu snapshot; không tự reclassify theo policy mới. |
